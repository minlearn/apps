...
wget https://raw.githubusercontent.com/minlearn/apps/master/_build/redriod/data.tar.gz -O data.tar.gz
tar zxf data.tar.gz
...
docker run -itd --privileged -v ./data/scrcpy-web/data:/data -v ./data/scrcpy-web/apk:/apk --name \${SCRCPY} --link \${REDROID} emptysuns/scrcpy-web:v0.1
...
docker run -d -v ./data/nginx/nginx.conf:/etc/nginx/nginx.conf -v ./data/nginx/passwd_scrcpy_web:/etc/nginx/passwd_scrcpy_web -v ./data/nginx/conf.d:/etc/nginx/conf.d -p 5555:80 --name \${NGINX} --link \${SCRCPY} nginx:1.24
